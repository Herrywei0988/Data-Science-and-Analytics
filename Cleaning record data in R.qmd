...
df2 <- df2 %>%
  mutate(housing_payment_pesos = as.numeric(housing_payment_pesos))
 
#INSERT CODE TO CONVERT "housing_payment_pesos" to USD
# 1 Mexican Peso = 0.050 USD
# 1 USD = 19.88 Mexican Peso
ddf2 <- df2 %>%
  mutate(housing_payment_pesos = as.numeric(housing_payment_pesos) * 0.050)
 
#INSERT CODE TO RENAME "housing_payment_pesos" to "housing_payment_usd"
df2 <- df2 %>%
  rename(housing_payment_usd = housing_payment_pesos)

# INSERT CODE TO MAKE SURE ALL "US" TAGS equal "usa" and all "MEX" tags equal "mex"
df2 <- df2 %>% 
mutate(country_of_origin = ifelse(country_of_origin == "US", "usa",
                            ifelse(country_of_origin == "MEX", "mex", country_of_origin)))
...
df_merged <- full_join(df1, df2, by = "customer_id")
...
missing_values <- colSums(is.na(df_merged))
print(missing_values) 

### INSERT CODE TO REMOVE THE COLUMNS "initials" AND "num_children", 
df_merged <- df_merged %>% select(-initials, -num_children) 
...
df_merged <- df_merged %>%
  mutate(across(c(customer_id, age_years, account_balance_usd, yearly_income_usd, housing_payment_usd), as.numeric))
...
mean_values <- df_merged %>% summarise(across(c(age_years, account_balance_usd, yearly_income_usd, housing_payment_usd), mean, na.rm = TRUE))
median_values <- df_merged %>% summarise(across(c(age_years, account_balance_usd, yearly_income_usd, housing_payment_usd), median, na.rm = TRUE))
std_values <- df_merged %>% summarise(across(c(age_years, account_balance_usd, yearly_income_usd, housing_payment_usd), sd, na.rm = TRUE))
...
df_merged <- df_merged %>%
  mutate(age_years = if_else(is.na(age_years), mean(age_years, na.rm = TRUE), age_years))

# INSERT CODE TO REPLACE ANY "NaN" in "yearly_income_usd" WITH THE MEDIAN

df_merged <- df_merged %>%
  mutate(yearly_income_usd = if_else(is.na(yearly_income_usd), median(yearly_income_usd, na.rm = TRUE), yearly_income_usd))
...
df_cleaned <- df_cleaned %>% distinct(customer_id, .keep_all = TRUE)
# PRINT THE DATAFRAME
print(df_cleaned)
```
...
df_cleaned <- df_cleaned %>%
  mutate(housing_payment_normalized = (housing_payment_usd - mean(housing_payment_usd, na.rm = TRUE)) / 
                                         sd(housing_payment_usd, na.rm = TRUE))
# INSERT CODE TO RENAME THE COLUMN "housing_payment_usd" --> "housing_payment_normalized"

df_cleaned <- df_cleaned %>% 
  select(-housing_payment_usd)
